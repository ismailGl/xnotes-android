...
class QuestionDetectionSession(
    ...
    fun accept(id: String, value: Boolean) {
        if (!busy && !saving) proposals = proposals.map { if (it.id == id) it.copy(accepted = value) else it }
    }
    fun acceptAll() { if (!busy && !saving) proposals = proposals.map { it.copy(accepted = !duplicate(it.sourcePageIndex, it.crop)) } }
    ...
    fun cancel() { if (!saving) job?.cancel() }
}
